	...
	if (pi->account_index == 0)
	{
		...
		else
		{
			...
			if (!row[3])
				...
				str_to_number(pSafebox->dwGold, row[3]);
			
			if (!row[4])
				pSafebox->dwCheque = 0;
			else
				str_to_number(pSafebox->dwCheque, row[4]);
#endif
			/*
			   if (!row[3])
			   pSafebox->dwGold = 0;
...
void CClientManager::QUERY_SAFEBOX_SAVE(CPeer * pkPeer, TSafeboxTable * pTable)
{
	...
	snprintf(szQuery, sizeof(szQuery),
#ifdef ENABLE_SAFEBOX_MONEY_SYSTEM
			"UPDATE safebox%s SET gold=gold,cheque=cheque WHERE account_id=%u", 
			GetTablePostfix(), pTable->dwID);
#else
...
void CClientManager::SafeboxMoney(TSafeboxMoney* pData)
{
	char szQuery[512];
	if (pData->bType)
		sprintf(szQuery, "UPDATE safebox%s SET `gold`=`gold`+%d WHERE account_id=%u LIMIT 1", GetTablePostfix(), pData->dwMoney, pData->dwAID);
	else
		sprintf(szQuery, "UPDATE safebox%s SET `gold`=`gold`-%d WHERE account_id=%u LIMIT 1", GetTablePostfix(), pData->dwMoney, pData->dwAID);
	...
	{
		sprintf(szQuery, "INSERT INTO safebox%s (account_id, gold) VALUES(%u, %d)", GetTablePostfix(), pData->dwAID, (pData->dwMoney>0 ? pData->dwMoney : 0));
...
void CClientManager::SafeboxCheque(TSafeboxCheque* pData)
{
	char szQuery[512];
	if (pData->bType)
		sprintf(szQuery, "UPDATE safebox%s SET `cheque`=`cheque`+%d WHERE account_id=%u LIMIT 1", GetTablePostfix(), pData->dwCheque, pData->dwAID);
	else
		sprintf(szQuery, "UPDATE safebox%s SET `cheque`=`cheque`-%d WHERE account_id=%u LIMIT 1", GetTablePostfix(), pData->dwCheque, pData->dwAID);
	...
	if (pData->bType && pMsg->Get()->uiAffectedRows == 0)
	{
		sprintf(szQuery, "INSERT INTO safebox%s (account_id, cheque) VALUES(%u, %d)", GetTablePostfix(), pData->dwAID, (pData->dwCheque>0 ? pData->dwCheque : 0));
		CDBManager::Instance().AsyncQuery(szQuery, SQL_PLAYER);
	}
